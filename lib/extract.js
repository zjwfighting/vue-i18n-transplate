...
function _extract(stack) {
    ...
    const strategy = {
        "template": (template) => {
                        ...
                        for (const prop of node.props) {
                            ...
                            if (prop.exp) {
                                ...
                                if (attrValue && /[\u4e00-\u9fa5]/g.test(attrValue)) {
                                    const matches = attrValue.match(/'[\s\S]+?'/g) || [];
                                    matches.forEach(match => {
                                        if (/[\u4e00-\u9fa5]/g.test(match)) {
                                            chineseTexts.push({ type: "PROP_COMPLEX", value: match.replace(/'/g, '') })
                                        }
                                    })
                                }
                            }
            ...
            ast.children[0] && traverse(ast.children[0]);
            return chineseTexts;
        },
        "script": (scripts) => {
                ...
                traverse(ast, {
                    StringLiteral(path) {
                        ...
                        if (chineseRegex.test(value)) {
                            chineseTexts.push({ type: "STRING_LITERAL", value })
                        }
                    },
...
function extract(target) {
    ...
    function processFile(filePath) {
        ...
        const chineses = _extract([
            ...
            { type: "script", content: content.indexOf("</script>") > -1 ? content.replace(scriptRegex, (_, _m1, m2) => m2) : "" }
        ]);
