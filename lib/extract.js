...
function extractFromScript(scriptContent) {
    ...
    try {
        ...
        console.log(fs.writeFileSync('./a.json', JSON.stringify(ast)))

        const texts = [];
        traverse(ast, {
            StringLiteral(path) {
                if (CHINESE_REGEX.test(path.node.value)) {
                    // 检查当前字符串是否位于一个 props 定义中
                    const isInsideProps = path.findParent((p) => {
                        // 1. 检查是否在 <script setup> 的 defineProps 中
                        if (p.isCallExpression() && p.get('callee').isIdentifier({ name: 'defineProps' })) {
                            return true;
                        }

                        // 2. 检查是否在 Options API 的 props 对象中
                        // 匹配 props: { ... }
                        if (p.isObjectProperty()) {
                            const key = p.get('key');
                            // 属性名是 'props'
                            if (key.isIdentifier({ name: 'props' }) || key.isStringLiteral({ value: 'props' })) {
                                // 确保这个 props 是导出对象的一部分，而不是一个普通变量
                                // p.parentPath.parentPath 可追溯到 ExportDefaultDeclaration
                                const grandParent = p.parentPath;
                                if (grandParent && grandParent.parentPath && grandParent.parentPath.isExportDefaultDeclaration()) {
                                    return true;
                                }
                            }
                        }
